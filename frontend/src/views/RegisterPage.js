...
const RegisterPage = () => {
  ...
  const [emailHasError, validateEmail, emailRef] = useFormInput((email) =>
    email.includes("@")
  );
  const usertypeRef = useRef();
  const [password1Ref, password2Ref, validatePassword, passwordError] =
    useFormPassword((password) => password.trim().length < 4);
        ...
        <form className={styles.registerForm} onSubmit={formSubmitHandler}>
          ...
          <div className={styles.gridRow}>
            ...
            {passwordError.hasError && (
              <p className="errorText">{passwordError.error}</p>
            )}
          </div>
